...
const openings: JobOpening[] = [
  ...
  },
  {
    ...
    location: "Manila",
    type: "Full-time",
    dialogKey: "seniorloan",
    roleDescription: "This is a full-time role for a Senior Loan Officer based in Manila. The Senior Loan Officer will lead loan origination, assessment, and disbursement activities. The role involves evaluating borrower creditworthiness, managing client relationships, mentoring junior loan officers, and ensuring compliance with lending policies and procedures.",
    qualifications: [
  ...
  },
  {
    ...
    type: "Internship",
    dialogKey: "intern",
    roleDescription: "This is an internship role for a Psychology / Sociology Intern based in Manila. The intern will support Datung's research initiatives in understanding borrower behavior, community dynamics, and psychometric profiling. The role involves assisting with data collection, analysis, and contributing to the development of innovative credit assessment models.",
    qualifications: [
...
const RiskDialog = ({ open, onOpenChange }: { open: boolean; onOpenChange: (v: boolean) => void }) => (
...
const StandardJobDialog = ({ job, open, onOpenChange }: { job: JobOpening | null; open: boolean; onOpenChange: (v: boolean) => void }) => {
  if (!job) return null;
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-4xl max-h-[80vh] overflow-y-auto bg-white">
        ...
        <div className="space-y-6">
          ...
          <div className="space-y-4 text-foreground">
            <div>
              ...
              <p className="mb-4">{COMPANY_DESC}</p>
            </div>
            <div>
              <h4 className="text-lg font-semibold mb-2">Role Description</h4>
              <p className="mb-4">{job.roleDescription}</p>
            </div>
            <div>
            ...
            <div className="bg-primary/20 border border-primary/30 p-4 rounded-lg text-center">
              ...
              <div className="flex gap-4 justify-center">
                {job.jobstreetUrl && (
                  <Button onClick={() => window.open(job.jobstreetUrl, '_blank')} className="bg-primary hover:bg-primary/90">Apply on JobStreet</Button>
                )}
      ...
      <div className="container mx-auto px-4">
        <div className="text-center mb-12">
          ...
          <p className="text-xl text-muted-foreground max-w-3xl mx-auto mb-8">
            We're looking for exceptional talent to join our mission. Every role at Datung is an opportunity to create meaningful impact.
          </p>
          ...
          {openings.map((job, index) => (
            <Card key={index} className="glass-card group card-3d">
              <CardContent className="p-8">
                ...
                <Button onClick={() => setActiveJob(job)} className="w-full group-hover:bg-primary/90" size="lg">View Details</Button>
              </CardContent>
            </Card>
          ))}
        </div>

        {activeJob?.isCustomDialog ? (
          <RiskDialog open={true} onOpenChange={(v) => { if (!v) setActiveJob(null); }} />
        ) : (
          <StandardJobDialog job={activeJob} open={activeJob !== null} onOpenChange={(v) => { if (!v) setActiveJob(null); }} />
        )}
      </div>
    </section>
  );
